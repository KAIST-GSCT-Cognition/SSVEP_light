    ...
    for label, label_data in enumerate(data):
        ...
        for trial, trial_data in enumerate(label_data):
            ...
            for chunk_idx in range(num_of_chunk):
                chunked_trial_data = trial_data[:, target_length * chunk_idx : target_length * (chunk_idx+1) ]
                normed_trial_data = channel_wise_norm(chunked_trial_data) # 6 x 1025
                # waveform (1) -> spec -> density
                delta_psd = welch_psd(normed_trial_data, SAMPLING_RATE, DELTA[0], DELTA[1])
                theta_psd = welch_psd(normed_trial_data, SAMPLING_RATE, THETA[0], THETA[1]) 
                alpha_psd = welch_psd(normed_trial_data, SAMPLING_RATE, ALPHA[0], ALPHA[1])
                beta_psd = welch_psd(normed_trial_data, SAMPLING_RATE, BETA[0], BETA[1])
                gamma_psd = welch_psd(normed_trial_data, SAMPLING_RATE, GAMMA[0], GAMMA[1])
                feature = {
                    "waveform": normed_trial_data,
                    "delta_psd": delta_psd,
                    "theta_psd": theta_psd,
                    "alpha_psd": alpha_psd,
                    "beta_psd": beta_psd,
                ...
                data_dictionary.append({
                    "id": f"{save_name}_{trial}_{label}_{chunk_idx}",
                    "pid": instance.pid,
                    "chunk_idx": chunk_idx,
